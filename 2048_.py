...
def moveleft(pi,pj,T):
    justcomb = False
    while pj>0 and (T[pi][pj-1] == 0 or (T[pi][pj-1] == T[pi][pj])):
        if T[pi][pj-1] == 0:
            T[pi][pj-1] = T[pi][pj]
            T[pi][pj] = 0
            pj-=1
        elif T[pi][pj-1] == T[pi][pj] and not justcomb:
            T[pi][pj-1] += T[pi][pj]
            T[pi][pj] = 0
            pj-=1
            justcomb = True
        elif T[pi][pj-1] == T[pi][pj] and justcomb:
            break
    return T    
def moveright(pi,pj,T):
    justcomb = False
    while pj<3 and (T[pi][pj+1] == 0 or (T[pi][pj+1] == T[pi][pj])):
        if T[pi][pj+1] == 0:
            T[pi][pj+1] = T[pi][pj]
            T[pi][pj] = 0
            pj+=1
